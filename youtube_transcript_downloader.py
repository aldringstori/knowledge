...
def main():
    ...
    # Sidebar for settings (only for admin)
    if check_password():
        st.sidebar.title("Admin Settings")
        new_download_folder = st.sidebar.text_input("Download Folder", value=config['download_folder'])
        if new_download_folder != config['download_folder']:
            config['download_folder'] = new_download_folder
            save_config(config)
            st.sidebar.success("Download folder updated successfully!")
    else:
        st.sidebar.title("Settings")
        st.sidebar.info("Login as admin to change settings.")

    # Main content area
    tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(["Single Video", "Single Short", "Channel Videos", "Channel Shorts", "Playlist", "File Converter"])
