#!/usr/bin/env python3
"""
Standalone robust ingestion script that doesn't rely on existing modules
"""
import os
import json
import time
...
LOG_FILE = os.path.join(LOG_DIR, "standalone_ingest.log")
...
class SimpleQdrantClient:
    """Simplified Qdrant client that directly accesses the SQLite database"""
    ...
    def _init_db(self):
        ...
        db_exists = os.path.exists(self.sqlite_path)
        ...
        try:
            ...
            conn.close()
            log(f"Successfully connected to SQLite database")
            return True
        except Exception as e:
            log(f"Error initializing SQLite database: {e}")
            return False

    def store_embedding(self, text, embedding, source, point_id):
        """Store embedding directly in SQLite database"""
        try:
            # Create payload
            payload = json.dumps({
                "text": text,
...
def process_file(file_path, qdrant_client, point_id):
...
def run_standalone_ingest():
    """Run standalone ingestion process"""
    # Initialize log
    with open(LOG_FILE, 'w', encoding='utf-8') as f:
        f.write(f"=== Standalone Ingest started at {datetime.now()} ===\n")
    ...
    # Initialize simplified Qdrant client
    log("Initializing simplified Qdrant client")
    qdrant_client = SimpleQdrantClient(qdrant_path)
